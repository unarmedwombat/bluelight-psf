<aside
    ...
    class="fixed inset-y-0 left-0 rtl:left-auto rtl:right-0 z-20 flex flex-col h-screen overflow-hidden transition duration-300 text-white w-80 lg:z-0 lg:translate-x-0"
>
    <header class="h-24 shrink-0 px-6 flex items-center">
    ...
    <nav class="flex-1 overflow-y-auto py-6">
        ...
        <ul class="space-y-6 px-6">
            @foreach (\Filament\Facades\Filament::getNavigation() as $group => $items)
                ...
                @if (! $loop->last)
                    <li>
                        <div class="border-t border-gray-500 -mr-6"></div>
                    </li>
                @endif
            @endforeach
        </ul>
